...
import { useGetFeature, useUpdateFeature, useListUnitOfMeasures } from '../../hooks/useFeature';
...
export const FeatureEdit: React.FC = () => {
  ...
  const { feature, loading: fetching, error: fetchError } = useGetFeature(id || '');
  const { updateFeature, loading: updating, error: updateError } = useUpdateFeature();
  ...
  useEffect(() => {
    if (feature) {
      setFormData({
        ...
        default_value: feature.default_value !== null && feature.default_value !== undefined 
          ? feature.default_value.toString() 
          : '',
        is_float: feature.is_float ?? false,
        unitOfMeasureFeaturesId: feature.unitOfMeasureFeaturesId || '',
      });
    }
  }, [feature]);
  ...
  if (fetchError || !feature) {
    return (
      <div className="bg-red-50 border border-red-200 rounded-lg p-4">
        <p className="text-red-800">Error: {fetchError || 'Feature not found'}</p>
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-gray-500">Update the feature information</p>
      </div>
